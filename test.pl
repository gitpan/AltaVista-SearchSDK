...
my $str = "This is a test document";
...
my $options = avs_create_options(100, 100, 1);
...
my $len = avs_search_getdatalen($search);
if ($len eq 23) {
    print "ok 13\n";
} else {
    $err = avs_errmsg($status);
...
my $docid = avs_search_getdocid($search);

my $msg = avs_search_getdata($search);
...
my $rel = avs_search_getrelevance($search);
if ($rel eq "1.000000") {
    print "ok 16\n";
} else {
    print "not ok 16\n";
